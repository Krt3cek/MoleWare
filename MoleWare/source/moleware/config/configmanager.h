...
namespace internal_config
{
    ...
    public:
        ...
        static void Save(const std::string& configName)
        {
            ...
            j["showDead"] = Config::showDead;
            j["showHead"] = Config::showHead;
            j["showWeapon"] = Config::showWeapon;
            j["headDotColor"] = {
                Config::headDotColor.x,
                Config::headDotColor.y,
                Config::headDotColor.z,
                Config::headDotColor.w
            };
            j["headDotRadius"] = Config::headDotRadius;
            ...
            j["aimbotTeamCheck"] = Config::aimbotTeamCheck;
            j["aimbot_fov"] = Config::aimbot_fov;
            j["aimbotSmoothness"] = Config::aimbotSmoothness;
            j["antiflash"] = Config::antiflash;
            j["rcs"] = Config::rcs;
            j["fov_circle"] = Config::fov_circle;

            j["triggerBot"] = Config::triggerBot;
            j["triggerBotDelay"] = Config::triggerBotDelay;
        ...
        static void Load(const std::string& configName)
        {
            ...
            Config::showDead = j.value("showDead", false);
            Config::showHead = j.value("showHead", false);
            Config::showWeapon = j.value("showWeapon", false);
            if (j.contains("headDotColor") && j["headDotColor"].is_array() && j["headDotColor"].size() == 4)
            {
                auto arr = j["headDotColor"];
                Config::headDotColor.x = arr[0].get<float>();
                Config::headDotColor.y = arr[1].get<float>();
                Config::headDotColor.z = arr[2].get<float>();
                Config::headDotColor.w = arr[3].get<float>();
            }
            Config::headDotRadius = j.value("headDotRadius", 1.0f);
            Config::distance = j.value("distance", false);
            ...
            Config::aimbotTeamCheck = j.value("aimbotTeamCheck", false);
            Config::rcs = j.value("rcs", false);
            Config::aimbot_fov = j.value("aimbot_fov", 0.f);
            Config::aimbotSmoothness = j.value("aimbotSmoothness", 0.f);

            Config::triggerBot = j.value("triggerBot", false);
            Config::triggerBotDelay = j.value("triggerBotDelay", 0);
