...
#include "includes.h"
#include "moleware/moleware.h"
#include "moleware/renderer/icons.h"
...
HRESULT __stdcall hkPresent(IDXGISwapChain* pSwapChain, UINT SyncInterval, UINT Flags)
{
    if (!init)
    {
        if (SUCCEEDED(pSwapChain->GetDevice(__uuidof(ID3D11Device), (void**)&pDevice)))
        {
            ...
            moleWare.init(window, pDevice, pContext, mainRenderTargetView);
            init = true;
        }
        else
    ...
    if (GetAsyncKeyState(VK_END) & 1) {
        moleWare.renderer.menu.toggleMenu();
    }

    moleWare.renderer.menu.render();
    moleWare.renderer.hud.render();

    // Always call esp() to allow individual components to be rendered
    moleWare.renderer.visuals.esp();
